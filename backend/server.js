...
server
  ...
  .post("/api/authenticate", async (req, res) => {
    const getUserAndPassword = await pool.query(
      `SELECT * FROM users WHERE username = $1 AND password = $2;`,
      [req.body.username, req.body.password]
    ); // query db for input username and password

    if (getUserAndPassword.rows.length === 0) {
      // if rows is empty, username and password combo does not exist
      res.send(false);
